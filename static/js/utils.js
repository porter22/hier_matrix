...
function create_uncovered_filter_div(highlight_checkbox_div, pairname, div_caption) {
  var inner_div = highlight_checkbox_div.append('div').attr('class','input_wrapper');

  inner_div.append('input')
                     .attr('id', pairname + '_rows')
                     .attr('type', 'checkbox')
                     .attr('class', pairname + ' checkbox')
                     .attr('value', pairname + '_rows')
  inner_div.append('label')
                      .attr('for', pairname + '_rows')
                      .text(div_caption)
}

//given filter type and to_highlight flag, highlight pair of rows that have no connections between each other
function highlight_unconnected_rows(filter_type, to_highlight) {

  console.log('highlighting unconnected rows')

  if (filter_type == 'sys_stake_high') {
    var source_rows = 'SYSTEM REQUIREMENTS'
    var target_rows = 'STAKEHOLDER REQUIREMENTS'
    var rel_type = 'satisfies'
  }

  if (filter_type == 'stake_sys_high') {
    var source_rows = 'STAKEHOLDER REQUIREMENTS'
    var target_rows = 'SYSTEM REQUIREMENTS'
    var rel_type = 'satisfies'
  }

  if (filter_type == 'sys_arch_high') {
    var source_rows = 'SYSTEM REQUIREMENTS'
    var target_rows = 'LEVEL1_ARCHITECTURE'
    var rel_type = 'allocated_to'
  }

  if (filter_type == 'arch_sys_high') {
    var source_rows = 'LEVEL1_ARCHITECTURE'
    var target_rows = 'SYSTEM REQUIREMENTS'
    var rel_type = 'allocated_to'
  }

  if (filter_type == 'sys_test_high') {
    var source_rows = 'SYSTEM REQUIREMENTS'
    var target_rows = 'TESTS'
    var rel_type = 'verified_by'
  }

  if (filter_type == 'test_sys_high') {
    var source_rows = 'TESTS'
    var target_rows = 'SYSTEM REQUIREMENTS'
    var rel_type = 'verified_by'
  }
  
  //select all rows with the where source rows have connections to target rows
    var selected_rows = d3.selectAll('.row').filter( function( d, i ) { 
      var cur_row_type = d3.select(this).attr('type')
      var result  = false  //do not change this row's highlight value
      if ( cur_row_type === source_rows) {
        result = true
        //console.log('SysReq row found')

        //if there are cells with rel_type.includes(satisfies)
        const cells = d3.select(this).selectAll('.cell')
        cells.each(function(){
          var cell = d3.select(this)
          if (cell.attr('link_type').includes(rel_type)) 
            result = false
        })
      }
      return result
  ...
  //increment or decrement highlight counter, depending on the highlight flag
  //we will later use this counter to determine if row should be highlighted
  selected_rows.each(function(d) {
    cur_highlight_count = parseInt(d3.select(this).attr('highlight_count'))
    
    if (to_highlight) {
      d3.select(this).attr('highlight_count', cur_highlight_count + 1)   
    } else {
      if (d3.select(this).attr('highlight_count') > 0)
      d3.select(this).attr('highlight_count', cur_highlight_count - 1)   
    }
  })

  //determine highlighting based on highlight counter
  selected_rows.each(function(d) {
    if (parseInt(d3.select(this).attr('highlight_count')) > 0)
      font_weight = 800
    else {
      font_weight = 400
    }
    
    console.log('affected rows:', d3.select(this).attr('name'), ' highlight count:', d3.select(this).attr('highlight_count'), ' font_weight:', font_weight)
    d3.select(this).style('font-weight', font_weight)
  }) 
}

//ANCHOR Highlight row on text click
function highlight_row(selectedindex) {
  ...
  var background_width = d3.select(".background").attr('width')

  //.. and highlight it
  this_or_next_row.select('line')
  .style('stroke','black')
  .attr('x2', background_width)
  .attr('is_highlighted','true')
...
function remove_empty_rows(original_order, cell_height, matrix, nodelist) {
  ...
  update_cells_translate(updated_order, nodelist, cell_height); //this should move cells along X
  
}

function resize_background (row_indexes, cell_height) {
  //shrink background
  ...
  //TODO quick fix
  d3.select('.background').attr('transform', "translate(5," + 0 + ")")
  var viewbox_width = new_bg_height + 100 + 160 //text shift and margin
  var viewbox_height = viewbox_width + 300 //it should be higher for 300, just like by default
  console.log('viewbox_width:', viewbox_width)
  console.log('viewbox_height:', viewbox_height)
  var viewbox_str = '0 0 ' + viewbox_width + ' ' + viewbox_height

  d3.select('svg').attr("viewBox", viewbox_str)

}

//goes through all the cells, and changes visibility of cells 
//.. within row_indexes array to the given visibility value
function change_cells_visibility (row_indexes, nodelist, visibility) {
  //hide cells corresponding to hidden rows
  row_indexes.forEach(function(row) {
    var nodename = nodelist[row].name
...
function update_cells_translate(order_list, nodelist, cell_height) {
  ...
  d3.selectAll('.cell').filter(function(d,i) {
    var target = d3.select(this).attr('target')
    var source = d3.select(this).attr('target')
    if (target === nodename || source === nodename) {
    //if (target === nodename) {
      //console.log('found the cell.')
      return true
...
function change_rows_visibility(checkbox_value, visibility, filter_type) {
  ...
  } else if (filter_type === 'aggr') {
    selected_rows = d3.selectAll('.row').filter(function(d,i) {
      const cells = d3.select(this).selectAll('.cell')
      var result  = true
      cells.each(function(){
        var cell = d3.select(this)
        console.log('cell rel type', cell.attr('is_aggregated'))
        if (cell.attr('is_aggregated') == 'aggregated') 
          result = false
      })
      return result
    })
...
function update_rows_translate(order_list) {
  ...
  console.log('update rows translate with list:', order_list)
  ...
  for (var i = 0; i < order_list.length; i++) {
    ...
    var upd_translate = i * cell_height //TODO: fix this
...
function update_cols_translate(order_list) {
  ...
  for (var i = 0; i < order_list.length; i++) {
    ...
    var upd_translate = i * cell_height + 5//TODO: fix this
      ...
      function draw_tree(root_data, height, width, svg) {
        ...
        */
        ...
        function getLinkColorType(source, target, links) {
            ...
            links.forEach(function(link) {
              ...
              source_ancestors.forEach(function(source_ancestor) {
                target_ancestors.forEach(function(target_ancestor) {
                  //if there is not already a link with such a source and target, then create new link
                  if (!is_in_links(links, source_ancestor, target_ancestor))
          ...
          //checks if there is already a link with such a source and target already
          function is_in_links(links, source, target) {
          ...
          function make_link(source, target, link_value) {
            ...
            new_link.is_aggregated = true

            //console.log('made new link:', new_link)
          ...
          */
          ...
          function showLegend(p) {
            ...
            var src = 'source:' + cur_cell.attr("source")
            var trg = 'target:' + cur_cell.attr("target")
            var type = 'type:' + cur_cell.attr("link_type")
          ...
          //prints a line of string to summary div
          function output_summary(line_string, div_class) {
            //select summary div
            var sum_div = d3.selectAll('#summary_div')
          ...
          //given node name, get that node's description
          function get_node_desc(nodelist, nodename) {
            console.log(nodelist, nodename);
          }
