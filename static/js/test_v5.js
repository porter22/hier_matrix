...
treeJSON = d3.json("./graph_nov21_from_matrix.json", function(error, json) {
  var clickedElemList = [];
  ...
  */
  ...
  svgGroup.attr("transform","translate(150,70)scale(.5,.5)");
...
var pixelmultiplier = 6; //width of label letter - needed to account for long labels
  ...
  //var lastPosY = 0;
    ...
    .attr("fill", function(d, i) {
      if (d.imglink == 'None' || !d.imglink) return 'lightsteelblue'
      else return 'green'
    })
    ...
    function highlightEdges(svgGroup, curtrace, paint) {
	  ...
	  function handleMouseOut(d, i) {
			...
			.attr("fill", function(d) {
					if (d3.select(this).attr('fill') == "red") {
						return "red";
					} else if (d.imglink == 'None' || !d.imglink){
  ...
  function collapse(nodeSelection) {
  	//console.log("current nodeSelection: ", nodeSelection);
        ...
        moveUp(nodeSelection);
  			nodeSelection.children = null;
  			nodeSelection.collapsed = true;
  		}
  }

//Minimizes the layout of the tree on collapse
//1. get hidden tree width
//2. move clicked element up to tree width
//3. find the next siblings of the clicked node, move them up to tree width recursively
  function moveUp(nodeSelection) {
    console.log("current nodeSelection: ", nodeSelection);
    console.log("startingY of the node's first child: ", nodeSelection.height);
    //move that node to lastPosY of its first child
    moveNodeVertically(svgGroup, nodeSelection, nodeSelection.height);
  }
